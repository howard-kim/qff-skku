...
const Hero = () => {
  ...
  // Custom countdown renderer (라벨도 i18n)
  const countdownRenderer = ({ days, hours, minutes, seconds, completed }) => {
    ...
    const Cell = ({ value, label, grad }) => (
      <div className="relative group">
        <div className={`absolute inset-0 bg-gradient-to-r ${grad} rounded-xl blur-lg opacity-40 group-hover:opacity-60 transition-opacity`} />
        <div className="relative bg-white border border-gray-200 rounded-xl p-3 md:p-4 shadow-lg">
          <div className={`text-2xl md:text-3xl font-bold bg-gradient-to-r ${grad} bg-clip-text text-transparent`}>
    ...
    return (
      <div className="flex justify-center gap-3 md:gap-6">
        <Cell value={days} label={t("countdown.days")}    grad="from-purple-600 to-blue-600" />
        <Cell value={hours} label={t("countdown.hours")}   grad="from-blue-600 to-cyan-600" />
        <Cell value={minutes} label={t("countdown.minutes")} grad="from-cyan-600 to-teal-600" />
        <Cell value={seconds} label={t("countdown.seconds")} grad="from-teal-600 to-green-600" />
      </div>
    );
  };
  ...
  return (
    <section className="relative bg-gradient-to-b from-white via-purple-50/30 to-white py-28 md:py-28 overflow-hidden">
      ...
      <div className="absolute top-0 right-0 w-96 h-96 bg-blue-200 rounded-full blur-3xl opacity-20 translate-x-1/2 -translate-y-1/2" />
      <div className="absolute bottom-0 left-1/2 w-80 h-80 bg-green-200 rounded-full blur-3xl opacity-20 -translate-x-1/2 translate-y-1/2" />

      {/* Floating particles */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-20 left-10 w-2 h-2 bg-purple-400 rounded-full animate-pulse" />
        <div className="absolute top-40 right-20 w-3 h-3 bg-blue-400 rounded-full animate-pulse animation-delay-1000" />
        <div className="absolute bottom-20 left-20 w-2 h-2 bg-green-400 rounded-full animate-pulse animation-delay-2000" />
        <div className="absolute bottom-40 right-10 w-3 h-3 bg-cyan-400 rounded-full animate-pulse animation-delay-3000" />
      </div>
        ...
        {/* Host */}
          <div className="mt-6 inline-block">
            <div className="flex flex-wrap justify-center items-center gap-3 text-xl sm:text-2xl md:text-3xl font-bold">
              <span className="text-gray-800">{t("host.prefix")}</span>
              <span className="relative">
                <span className="absolute inset-0 bg-gradient-to-r from-green-600 to-emerald-600 blur-xl opacity-40" />
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-6 text-gray-600">
            <div className="flex items-center gap-2">
              ...
              <span className="font-medium">{t("details.date")}</span>
            </div>
            <div className="flex items-center gap-2">
              <svg className="w-5 h-5 text-purple-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <span className="font-medium">{t("details.place")}</span>
            </div>
            <div className="flex items-center gap-2">
              <svg className="w-5 h-5 text-purple-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
              </svg>
              <span className="font-medium">{t("details.tagline")}</span>
            </div>
          </div>

          {/* Countdown */}
          <div className="mt-10">
            <Countdown date={new Date(t("countdown.targetISO"))} renderer={countdownRenderer} />
          </div>

          {/* CTA Buttons */}
          <div className="mt-10 flex flex-wrap justify-center gap-4">
            <a
              href={t("cta.register.url")}
              target="_blank"
